...
def main(args):
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, collate_fn=dataset.collate_fn, shuffle=False)
    ...
    with torch.no_grad():
        for batch in dataloader:
            encoded = batch["text"].to(args.device)
            ids = batch["id"]

            preds = model(encoded)
            for pred, id in zip(preds, ids):
                _, pred_label = torch.max(pred, dim=0)
                pred_list.append(pred_label.squeeze())
                id_list.append(id)
    
    pred_list = [dataset.idx2label(index.item()) for index in pred_list]
    # COMPLETE
